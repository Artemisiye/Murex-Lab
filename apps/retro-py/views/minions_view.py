...
class MinionsView(BaseView):
    ...
    def handle_event(self, event, mx, my):
        """Internal handler for custom logic not captured by widgets."""
    ...
    def draw_view(self, buffer, context):
        """Standard draw loop with context-aware state access."""
        # 1. Base widget tree draw
        super().draw_view(buffer, context)

        # 2. Dynamic content from central state
        state = context.get('state', {})
        player_minions = state.get('player_minions', [])
        ...
        if player_minions:
            ...
            if body_f:
                ...
                body_f.draw(buffer, "CLASS: ARTIFICER", 32, 88, styles.get_color("fg"))
